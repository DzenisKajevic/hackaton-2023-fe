...
import { loginToProfile } from '../api/profileAuth';
...
const LoginProfile = () => {
  ...
  const profileLoginCall = async () => {
    const arg1 = localStorage.getItem("profileToLogInto");
    console.log(arg1);
    const pass = document.getElementById('password').value;
    console.log(pass);
    const result = await loginToProfile(arg1, pass);
  ...
  return (
    <div className='flex flex-col register-form h-[100vh] my-auto flex-auto items-center justify-center bg-primary'>
      ...
      <div className='px-[40px] xs:px-0 ss:w-5/6 md:w-1/2 flex flex-col'>
        <h2 className='text-[34px] font-bold text-secondary text-center mb-[20px] leading-[30px]'>Login into your profile</h2>
        <p className='text-[16px] text-text text-center mb-[50px]'>Please enter info to successfully login.</p>
        ...
        <button type='submit' onClick={ profileLoginCall } className='text-[14px] text-secondary text-center mb-[20px]'>Login into profile</button>
